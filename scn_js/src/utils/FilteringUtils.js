...
export function getFilteredIndices(plotData, fields) {
    let predicate = (i) => {
        ...
        for (let key in fields.factorLevels) {
            if (!_.includes(fields.factorLevels[key], x[key])) {
                console.log(x, key, fields.factorLevels[key])
                return false;
            }
        }
        for (let key in fields.numericRanges) {
            if (fields.numericRanges[key][0] > x[key] || x[key] > fields.numericRanges[key][1]) {
                console.log(x, key, fields.numericRanges[key])
                return false;
            }
        }
        return true;
