...
import {ViolinPlotComponent} from "./PlotComponents";


const _DatasetViolinInputs = ({token, tab, x, split, asIs, scaled, log2, zscore, fields, isGeneLoading, geneResults, geneValue, dispatch}) => {
    return (<Grid.Column width={3}>
        <Form>
            <Divider horizontal>Coordinates</Divider>
            <DropDownComponent label={"X coordinate"} name={'x'} value={x} options={getFields(fields.factor)} onChange={(e, { name, value }) => dispatch(inputChanged(token, tab, name, value))} />
...
const mapInputStateToProps = (state, ownProps) => {
    ...
    return {
        ...
        x, split, asIs, scaled, log2, zscore, isGeneLoading, geneResults, geneValue
    }
};

const DatasetViolinInputs = connect(
    mapInputStateToProps,
    null
)(_DatasetViolinInputs);
